...
==================================================
   ...
   Author :       sjyttkl
   date：          2019/8/4
   Description :  Transformer train.py 训练数据
==================================================
"""
__author__ = 'sjyttkl'
    ...
    def train(self):
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                transformer = model.Transformer(self._config,wordEmbedding=self.wordEmbedding)
                ...
                loss_summary = tf.summary.scalar("loss",transformer.loss)
                acc_summary = tf.summary.scalar("accuracy", transformer.accuracy)
                ...
                def train_step(batchX,batchY):
                    ...
                    _, summary, step, loss,accuracy, predictions, binaryPreds = sess.run(
                        [train_op, train_summary_op, global_step, transformer.loss,transformer.accuracy, transformer.predictions, transformer.binaryPreds],
                        feed_dict)
                ...
                def devStep(batchX, batchY):
                    ...
                    summary, step, loss, accuracy, predictions, binaryPreds = sess.run(
                        [ dev_summary_op, global_step, transformer.loss, transformer.accuracy, transformer.predictions,
                          transformer.binaryPreds],
                        feed_dict)
                    timeStr = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())#datetime.datetime.now().isoformat()
                    acc, auc, precision, recall = data_helper.genMetrics(batchY, predictions, binaryPreds)
